...
public class CachedPeopleDaoTest {
  ...
  public void testListAll() {
    List<Person> people = cachedPeopleDao.listAll();
    Assert.assertTrue(compareLists(myPeople.values().stream().toList(), people));
  }
  ...
  public void testSave() {
    ...
    Assert.assertTrue(compareLists(myPeople.values().stream().toList(), cachedPeopleDao.listAll()));
  }

  @Test
  public void testDelete() {
    if (cachedPeopleDao.listAll().isEmpty() && myPeople.isEmpty()) return;
    UUID id = cachedPeopleDao.listAll().get(0).getId();
    try {
